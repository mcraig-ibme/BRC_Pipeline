#!/bin/bash
#
# Version of ASL processing using oxasl
#
...
oxasl -i ${aslMRIrawFolder}/${OrigASLName} \
      -s ${dataT1Folder}/T1_unbiased \
      --sbrain ${dataT1Folder}/T1_unbiased_brain \
      --wm_seg=${SinChanT1Folder}/T1_WM_mask \
      --wm_pve=${SinChanT1Folder}/T1_pve_WM \
      --gm_pve=${SinChanT1Folder}/T1_pve_GM \
      --output=${aslFolder} \
      >>"${logFile}" 2>>"${logFile}"

#      --casl --iaf=tc --ibf=tis --slicedt=0.0452 \
#      --plds=0.25,0.5,0.75,1.0,1.25,1.5 --bolus=1.4 \
#      --fslanat $DATA/T1.anat --senscorr \
#      -c $DATA/aslcalib --tr=4.8 --cmethod=refregion \
#      --csf=$DATA/csfmask --tissref=csf \
#      --cblip=$DATA/aslcalib_PA --echospacing=0.00952 --pedir=y \
#      --mc \
#      --pvcorr \
#      --region-analysis \
#      -o oxasl_mpld_out --save-preproc --save-corrected --save-corrections --overwrite --debug
